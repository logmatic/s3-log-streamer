...
function S3Poller (options) {
	...
	//Store s3_params
	if (!(options.s3_params && options.s3_params.Bucket && options.s3_params.Prefix)) {
		throw Error('s3_params not properly defined. Bucket & Prefix are mandatory.');
	}
	this._s3_params = options.s3_params;

	//Last cursors
	if(options.last && !(options.last.Key && options.last.size)){
		throw Error("When 'last' is defined. Key & Size becomes mandatory");
	}
...
S3Poller.prototype.poll = function () {
	var s3 = new AWS.S3();
	var self = this;
	...
	var readNextFile = function () {
		...
		if (nextFile == null) { // Need to lookup the next files
			...
			var listParams = {
				...
				Prefix: self._s3_params.Prefix,
			};
			if (self._last.Key) {
				listParams.Marker = self._last.Key.substring(0,self._last.Key.length-1);
			} else if (self._s3_params.Marker) {
				listParams.Marker = self._s3_params.Marker;
			}
	...
	var handleS3Content = function(content,callback){
		...
		var objectParams = {Bucket: self._s3_params.Bucket,Key: content.Key};
		...
		if(self._last
			...
			rstream.on('error',function(err){logger.error("Error while calling getObject on AWS S3",err);callback();});
			rstream.on('end',function(){logger.debug("END object",content.Key);callback();});

			//Emit lines
			new Lazy(rstream).lines.forEach(function(line){
				//Build json object
				var jsonObject = {
					s3bucket: self._s3_params.Bucket,
					s3key: this.Key,
